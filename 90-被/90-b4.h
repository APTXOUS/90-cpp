...
const LPBYTE  byRSExp[] = { NULL,      NULL,      NULL,      NULL,      NULL,      NULL,      NULL,      byRSExp7,  NULL,      NULL,
...
public:
	char *target;			//原数据
	int target_num;			//原数据数量
	bool *ans;				//编码（bool型）
	unsigned char *code;	//编码(数字)
	int len_num_char;		//编码数量(数字)
	int len_num;			//编码数量
	int version;			//版本号
	int version_group;		//版本类别
	int version_level;		//纠错级别
	int version_num;		//版本最大数据量
	int num_code_word_final;//最终二维码编码数量
	unsigned char m_byRSWork[153];// rs 码字计算工作
public:
	unsigned char *code_word_final;//最终二维码编码
	int num_code_word_without;		//最终数据码
	unsigned char module_code[177][177];//最终二维码
	int m_nSymbleSize;
public:
	QRcode();
	~QRcode();
	bool encode_qrcode(const char *, int);
	char* G2U(const char* gb2312);		//gbk转utf—8
	bool encode_byte();					//转字节模式
	void add_zeros();					//补0
	void add_what();					//补字符
	int QRcode::auto_version();			//判断版本号
	void QRcode::auto_version_group();	//判断版本组
	void add_code_data();
	void change_bool_into_char();
	void QRcode::encode_rscode(LPBYTE , int , int );
	//*************************************************
	void add_rs_code_data();
	void QRcode::FormatModule();
	void QRcode::SetFunctionModule();
	void QRcode::SetFinderPattern(int, int);
	void QRcode::SetAlignmentPattern(int, int);
