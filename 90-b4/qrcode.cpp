...
// 用  途：数据编码
// 引  数：纠错级别、类型号 (0 = 自动)、auto-extension 标志、掩码数 (-1 = 自动)、编码数据、编码数据长度
// 戻り値：编码成功 = true、无数据或超过容量 = false
...
bool CQR_Encode::EncodeData(int nLevel, int nVersion, bool bAutoExtent, int nMaskingNo, LPCSTR lpsSource, int ncSource)
{
	...
	else
	{
		...
		{
			if (bAutoExtent)
				m_nVersion = nEncodeVersion; // 版本 (型号) auto-extension
			else
				return false; // 超过容量
		}
	}
	...
	system("pause");
	for (int k = 0; k < ncDataCodeWord; k++)
		printf_s("%o", m_byDataCodeWord[k]);
	system("pause");
	/*char a[] = "01000000100111100101101100001010010011100101101100001010011111100101101011111000010100001110110000010001";
	int num_a = strlen(a);
	int j_ = 0;
	for (int i = 0; i < num_a; i = i + 8)
	{
		...
		if (a[i + 0] == '1')
			num += 128;
		m_byDataCodeWord[j_] = num;
		j_++;
	}
	system("pause");
	for (int k = 0; k < ncDataCodeWord; k++)
		printf_s("%x", m_byDataCodeWord[k]);
	system("pause");
	...
	*/
	...
	system("pause");
	for (int k = 0; k < m_ncAllCodeWord; k++)
		printf_s("%o", m_byAllCodeWord[k]);
	system("pause");
...
bool CQR_Encode::EncodeSourceData(LPCSTR lpsSource, int ncLength, int nVerGroup)
{
	...
	m_ncDataBlock = ncLength;
	int ncSrcBits, ncDstBits;

	while (m_ncDataBlock - 1>0)
	{
		ncDstBits = GetBitLength(QR_MODE_8BIT, m_nBlockLength[0] + m_nBlockLength[1], nVerGroup) - (4 + nIndicatorLen8Bit[nVerGroup]);
		m_nBlockLength[0] += m_nBlockLength[1];

		// 後続をシフト
		for (i = 1; i < m_ncDataBlock - 1; ++i)
		{
	...
	/////////////////////////////////////////////////////////////////////////
	// 位排序
	int ncComplete = 0; // 已处理的数据计数器
	WORD wBinCode;

	m_ncDataCodeWordBit = 0; // 按位处理计数器
...
// 用法: 位长获取
//参数: 数据模式类型、数据长度、版本 (型号) 组
//返回: 数据位长度
//注释: 日文汉字模式中的数据长度参数, 而不是字符数
...
// 使用: 位设置
//参数: 插入位置, 位数组数据, 数据位长 (最多 16)
//返回值: 下一个插入位置 (缓冲区超过 =-1)
//备注: 将结果设置为 Mebydatacodeword (需要零初始化)
...
void CQR_Encode::GetRSCodeWord(LPBYTE lpbyRSWork, int ncDataCodeWord, int ncRSCodeWord)
{
	...
	for (i = 0; i < ncDataCodeWord; ++i)
	{
		if (lpbyRSWork[0] != 0)
		{
			BYTE nExpFirst = byIntToExp[lpbyRSWork[0]]; // 从第一项系数计算乘数

			for (j = 0; j < ncRSCodeWord; ++j)
			{
				// 将第一个术语乘数添加到每个乘数 (% 255 →α ^ 255 = 1)
